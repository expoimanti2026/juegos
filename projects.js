const projects = [
{
        alumno: "Maor Averbuch",

        proyecto: "tahiti redemption",

        scratch: "https://scratch.mit.edu/projects/1295527998/embed",

        trailer: "",

        instrucciones: `
espera hasta 1 para disparar
space = shoot
`
    },
{
        alumno: "Antonio Barajas",

        proyecto: "Escapa del Hipopotamo-Mosca",

        scratch: "https://scratch.mit.edu/projects/1289747332/embed",

        trailer: "",

        instrucciones: `
A,D y Espacio
`
    },
{
        alumno: "Nabila Barba",

        proyecto: "Musa",

        scratch: "https://scratch.mit.edu/projects/1310622873/embed",

        trailer: "",

        instrucciones: `
usa la flecha para avanzar y llegar al castillo sin tocar la bomba, al tocar la bomba volveras al mismo lugar de antes

<a href="https://www.youtube.com/watch?v=DNlb-iRQj3I" target="_blank" rel="noopener noreferrer">https://www.youtube.com/watch?v=DNlb-iRQj3I</a>

<a href="https://www.youtube.com/watch?v=BpBvCRArQtc" target="_blank" rel="noopener noreferrer">https://www.youtube.com/watch?v=BpBvCRArQtc</a>
`
    }, 
{
        alumno: "Diana Campos",

        proyecto: "pelea con un perro ",

        scratch: "https://scratch.mit.edu/projects/1330533591/embed",

        trailer: "",

        instrucciones: `
Pelea con un perrro de la calle o se el perro
Perro:
A: ir a la izquierda
D : ir a la derecha
W : atacar

Persona:
Flecha a la izquierda : ir a la izquierda
Flecha a la derecha : ir a la derecha
Flecha arriba : atacar
`
    }, 
{
        alumno: "Damián Carsolio",

        proyecto: "Laberinto",

        scratch: "https://scratch.mit.edu/projects/1280898738/embed",

        trailer: "https://drive.google.com/file/d/1xod0YsuDtnaSiN2buQpEEkqjMbCj3F_q/preview",

        instrucciones: `
Usa las flechas para mover al Pacman, si tocas el borde del laberinto perderás una vida y regresaras hasta el punto de inicio, si tocas al guardia perderás automáticamente. Cuando logres obtener todos los tacos ganarás.

`
    },
{
        alumno: "Paulina Flores",

        proyecto: "Sugar Rush",

        scratch: "https://scratch.mit.edu/projects/1310606296/embed",

        trailer: "https://drive.google.com/file/d/1q-s0ZXg1ChrlgfLSsAzYpJn4N3ACj91l/preview",

        instrucciones: `
1. Elige tu carro
2. Para jugar se ocupa la tecla de espacio para avanzar el carro
3. La flecha de la izquierda sirve para dar vuelta a la izquierda.
4. Con la flecha izquierda puedes dar vuelta a la derecha.
5. Al ganar dale click al check point (la corona) para recibir tu recompensa.

<b>Gracias por jugar :)<b>
`
    },
     {
        alumno: "Lorenzo Frias",

        proyecto: "Harvesting Game",

        scratch: "https://scratch.mit.edu/projects/1335039886/embed",

        trailer: "https://drive.google.com/file/d/13lVkR23KFr7zuBCMB4yEa86f-U7_TRPd/preview",

        instrucciones: `
Tráele las verduras a la manzana.
AESTHETIC ASMR WARNING
`
    },
    {
        alumno: "Emily Galván",

        proyecto: "SHARKKK.23",

        scratch: "https://scratch.mit.edu/projects/1310641515/embed",

        trailer: "https://drive.google.com/file/d/1FZ9W4HWupJrPAX5xemQaEmZRkFdyJXUH/preview",

        instrucciones: `
Pasa por en medio de los barrotes saltando con la tecla "espacio" y pícale a la "A" para reiniciar el juego y poder empezar desde cero para ganar más puntos

`
    },
{
        alumno: "Santiago García",

        proyecto: "SPACING JUMP!",

        scratch: "https://scratch.mit.edu/projects/1310634372/embed",

        trailer: "https://drive.google.com/file/d/1i8GD78D-0u7qFFp5TczU8wIRTberyBjT/preview",

        instrucciones: `
Eres un astronauta perdido en el espacio después de que la misión fallara y la nave explotara . Estas solo, los demás compañeros no sobrevivieron de la explosión de la nave, por lo que tendrás que ir a casa, pero ten cuidado, las estrellas son peligrosas, por lo que tendrás que tener mucho cuidado. ¿Podrás volver a casa sabiendo las consecuencias?

Para iniciar el juego, presiona cualquier tecla, y para saltar, usa la tecla espacio.
`
    },
{
        alumno: "Ian Gómez",

        proyecto: "TAHITI REDEMPTION V2",

        scratch: "https://scratch.mit.edu/projects/1310614356/embed",

        trailer: "",

        instrucciones: `
Eres un vaquero en el viejo oeste y tienes que enfrentarte contra tu peor enemigo RIP VAN WINKLE es un duelo demasiado rapido
Lo hice algo tardado ya que varias veces el codigo no funciono y no puedo asegurar en un 100 porciento que funcione muy bien
`
    },
{
        alumno: "Reneé González",

        proyecto: "Be a turtle",

        scratch: "https://scratch.mit.edu/projects/1310614942/embed",

        trailer: "https://drive.google.com/file/d/1Kt5xH3ZlnXbDMCpLeFMbELNKIm3Q-r8G/preview",

        instrucciones: `
Eres una tortuga, tu misión será tomar decisiones para vivir y hacer amigos por todo el oceano en donde vives. :3
`
    },
{
        alumno: "Camila Gutiérrez",

        proyecto: "Pusheen",

        scratch: "https://scratch.mit.edu/projects/1310630119/embed",

        trailer: "",

        instrucciones: `
atrapa l estrella

`
    },
    {
        alumno: "Liam Halsey",

        proyecto: "raise a kitty",

        scratch: "https://scratch.mit.edu/projects/1310623188/embed",

        trailer: "https://drive.google.com/file/d/1DXjFO7IwyIsC5n-jpQpT8BbP1b1sxwmq/preview",

        instrucciones: `
cuida a kitty y dale de comer presiona a tu kitty para obtener dinero

`
    },
{
        alumno: "Mía Jacobo",

        proyecto: "Ody's Odyssey",

        scratch: "https://scratch.mit.edu/projects/1283651828/embed",

        trailer: "https://drive.google.com/file/d/1UXcmq-q6yajVj0mOL7i4m31jMxyUk20M/preview",

        instrucciones: `
Fight monsters and try to get home to your wife.

¿Alguien de aquí ya vio Epic: The Musical? (Probablemente no obviamente)

`
    },
{
        alumno: "Miguel Lizarraga",

        proyecto: "Un chef en apuros",

        scratch: "https://scratch.mit.edu/projects/1310614903/embed",

        trailer: "",

        instrucciones: `
El jugador controla a un chef que debe atrapar ingredientes correctos para completar recetas mientras evita comida en mal estado, cada nivel representa una receta diferente con mayor dificultad.
`
    },
{
        alumno: "Nina Macfarland",

        proyecto: "PJO INVADERSSSS",

        scratch: "https://scratch.mit.edu/projects/1310631477/embed",

        trailer: "",

        instrucciones: `
Tienes que usar las teclas "w" y "s" para disparar tus unicas 2 cañones que tienes y le tienes que atinar a los dos monstruos. Al final avanza hasta llegar a la orilla y ganaras el juego.`
    },
{
        alumno: "Samuel Marquez",

        proyecto: "Cuphead Pirata 2",

        scratch: "https://scratch.mit.edu/projects/1316695833/embed",

        trailer: "https://drive.google.com/file/d/1yNIFFPoYinBL9KEgWtU3h6o_lxf4suyN/preview",

        instrucciones: `
Usa espacio para saltar
Puedes usar AD o las flechas para moverte de un lado al otro.

Es FRAMEPERFECT
`
    },
{
        alumno: "Genaro Mendoza",

        proyecto: "Caldo de Res Runner",

        scratch: "https://scratch.mit.edu/projects/1295540839/embed",

        trailer: "https://drive.google.com/file/d/1YEvyyfQTCyY66G4rKSkZcvzmN1mnCtX8/preview",

        instrucciones: `
Help Thukuna to get his "Caldo de Res" you can move to left and right with "a" and "d", be careful with some trains because they may have a bomb and be able to hit you from far away.

You can also get the magnet for an extra coin bonus. The game is endless, poor Thukuna, but you can loose.

`
    },
{
        alumno: "Lou Molodij",

        proyecto: "Geometry Dash: scratch",

        scratch: "https://scratch.mit.edu/projects/1329783354/embed",

        trailer: "https://drive.google.com/file/d/1CAgflGPNuG9D6fT2kPiZs7kaZbqcxCtE/preview",

        instrucciones: `
¡Hola! Aquí tenéis una versión Scratch del nostálgico juego Geometry Dash. Cómo jugar: tenéis que usar la barra espaciadora de vuestro ordenador para que el personaje salte. Tenéis que saltar sobre los obstáculos o esquivarlos para aguantar el mayor tiempo posible. Como en el Geometry Dash original, el nivel se reinicia. No hay muchos niveles, pero pronto añadiremos más. ¡Que os divirtáis!

Música: "Stereo Madness" de RopTop Games
`
    },
{
        alumno: "Omar Obeso",

        proyecto: "The Dare",

        scratch: "https://scratch.mit.edu/projects/1319763768/embed",

        trailer: "https://drive.google.com/file/d/1vzY2tOQCKQtkl1Imn3V21plAGzUPGsp4/preview",

        instrucciones: `
Crédito a _LC12 por el proyecto original
<a href="https://scratch.mit.edu/projects/732050158" target="_blank" rel="noopener noreferrer">Casual 3D Shooter</a>

W , D - Move
Mouse -shoot
`
    },
{
        alumno: "Dayani",

        proyecto: "a̶̴̸̡̮͙̳͍͖̜̝̝̱̙̯̺̞͐͆̍ͣ͐̔̐̔͆ͨ́̾̃̃͘͟͢͠͝d̶̛͕̫̲̻͚̈́̔ͥͣ͌̍̆̂̒ͬ͢͟͠͠ͅo̡̡̢̤̼̲̻̬̰̖͙̲͓̤͉̱̯̰͛͑̎͊ͣͬ̀̾̐̃́̽ͦͣͬ̄̓̎̕͠",

        scratch: "https://scratch.mit.edu/projects/1316699245/embed",

        trailer: "",

        instrucciones: `
Todavía esta terminado, los textos no tienen significado, son letras al azar NO ESTA TERMINADO

Música: Omori ost, Takopi no genzai ost, mothy, 32ki

Las imágenes son hechas por mi
`
    },
{
        alumno: "Ximena Palafox",

        proyecto: "Pelota",

        scratch: "https://scratch.mit.edu/projects/1310619101/embed",

        trailer: "https://drive.google.com/file/d/1iUlAeOjfc2E7I5b9puJ_VyXJR-lxGbTZ/preview",

        instrucciones: `
solo juega nose bueno es para 2 jugadores uno es las teclas a y d y el otro las flechitas
`
    },
{
        alumno: "Luis Rivera",

        proyecto: "Siderman",

        scratch: "https://scratch.mit.edu/projects/1310628797/embed",

        trailer: "https://drive.google.com/file/d/1NXxL8GqyUfj2L03kUmM3HQXOU_2HD0Nx/preview",

        instrucciones: `
`
    },
{
        alumno: "Lia Salazar",

        proyecto: "el gran salto",

        scratch: "https://scratch.mit.edu/projects/1328182838/embed",

        trailer: "https://drive.google.com/file/d/1YWdJ66AOFaV5iJoStAD-7JhkN4sH8EYW/preview",

        instrucciones: `
saltar sin tocar el bicho
`
    },
{
        alumno: "Kedwin Sánchez",

        proyecto: "Correr sobre un alien",

        scratch: "https://scratch.mit.edu/projects/1330554370/embed",

        trailer: "https://drive.google.com/file/d/1EReNsF-Yhm-m20vOyJp83jYvYsZ2k8oy/preview",

        instrucciones: `
Escapa sobre un alien que quiere parate para que no agarres ningun planeta

tecla A: izquierda
tecla D: derecha
tecla W: arriba
tecla S: abajo
`
    },
{
        alumno: "Ana Sánchez",

        proyecto: "Pusheen",

        scratch: "https://scratch.mit.edu/projects/1325652859/embed",

        trailer: "",

        instrucciones: `
Este juego es un clicker de Pusheen que cambia de accesorios cada 10 clicks

`
    },
{
        alumno: "Sky Steyn",

        proyecto: "Escape",

        scratch: "https://scratch.mit.edu/projects/1319772674/embed",

        trailer: "https://drive.google.com/file/d/1JqoWbxV6Wauf_S8wDYCChjPIxkNgEo_Q/preview",

        instrucciones: `
Welcome to *Escape* you were summoned onto a new galaxy where you need to escape avoiding obstacles like spikes and other things in order to come back to life.

How to play:
Arrow Keys
Mouse pad
`
    },
{
        alumno: "Roberta Turrent",

        proyecto: "bloom",

        scratch: "https://scratch.mit.edu/projects/1316690384/embed",

        trailer: "",

        instrucciones: `
utiliza la flecha para avanzar y llegar al castillo. No toques la bola de fuego si la tocas volverás al mismo lugar

<a href="https://youtu.be/3CuVGq2Oo7M?si=tkfXCAr0qY90BauX" target="_blank" rel="noopener noreferrer">https://youtu.be/3CuVGq2Oo7M?si=tkfXCAr0qY90BauX</a>

`
    }
]